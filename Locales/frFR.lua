
if GetLocale() ~= "frFR" then return end
local _, mod = ...
local L = mod.L

-- Core
--L.firstRunWarning = "Timers will not be shown until you see your first invasion."
L.invasionsAvailable = "|T236292:15:15:0:0:64:64:4:60:4:60|t Nouvelles invasions disponibles !"
L.tooltipClick = "|cffeda55fClique-gauche|r pour glisser et déplacer."
L.tooltipClickOptions = "|cffeda55fClique-droit|r pour ouvrir les options."
--L.mode = "Mode"
--L.modeBar = "Bar"
--L.modeBroker = "Data Broker"

-- Bosses
L.runToBoss = "Courrez vers le boss !"
L.runAwayFromBoss = "Ecartez-vous du boss !"
L.frontConeDmg = "Frontal cone damage!"
L.frontalCleave = "Frontal Cleave!"
L.frontalCleaveWipe = "Frontal Cleave - Risque de wipe !"
L.debuffInc = "Un débuff arrive !"
L.mindControlInc = "Un sort de contrôle d'esprit arrive !"
L.threatWipeSpawnAdd = "Risque de wipe - Spawn d'add !"
L.killOrbsFast = "Tuez les orbes rapidement !"
L.sharedCleave = "Shared Cleave - Placez vous devant le boss !"
L.finished = "%s s'est terminé !"
L.dispelBoss = "Dispel boss!"
L.watchOut = "%s est apparu, ATTENTION !"
L.changeTank = "%s sur %s, changez de tank !"
L.damageMark = "Marque sur vous, évitez les dégats !"
L.runToLink = "Courrez vers le joueur lié"
L.healYourself = "Debuff - Soignez-vous !"
L.runOut = "%s sur vous, fuyez !"
L.keepMoving = "%s sur vous, continuez à courrir !"
L.firePatches = "Vous lâchez des gerbes de flammes !"
L.fireBomb = "Bombe ! Restez écartés de 10 mètres"
L.ballOnYou = "Vous êtes fixé par une Boule Funeste, fuyez !"
L.soakMeteor = "Rapprochez-vous des autres joueurs pour partager les dégâts du météore !"
